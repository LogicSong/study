...
而**严格模式下不会指定全局对象为默认调用对象，所以答案是 undefined**。
...
要实现这个要求，我们必须知道函数相关的两个隐式转换函数 toString() 和 valueOf()。t**oString() 函数会在打印函数的时候调用**，比如 console.log、**valueOf 会在获取函数原始值时调用**，比如加法操作。
...
简单地理解，原型就是对象的属性，包括被称为**隐式原型的 proto 属性**和被称为**显式原型的 prototype 属性**。
...
![image-20210228132450881](/Users/hqer/Library/Application Support/typora-user-images/image-20210228132450881.png)
